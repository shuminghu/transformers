...
class PerceptionLMConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`PerceptionLM`]. It is used to instantiate an
    ...
    ```python
    >>> from transformers import PerceptionLM, PerceptionLMConfig, SiglipVisionConfig, Qwen2Config
    ...
    >>> # Initializing a PerceptionLM perception-lm-hf/perception-lm-qwen2-7b-ov-hf style configuration
    >>> configuration = PerceptionLMConfig(vision_config, text_config)

    >>> # Initializing a model from the perception-lm-hf/perception-lm-qwen2-7b-ov-hf style configuration
    >>> model = PerceptionLM(configuration) 
